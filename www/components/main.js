...
function calcRoute() {
    ...
    directionsService.route(request, function (result, status) {
        if (status == google.maps.DirectionsStatus.OK) {
            ...
            $("#output").html("<div class='result-table'> Driving distance: " + formattedDistance + ".<br />Tempo Médio: " + formattedDuration + ".<br />Custo: " + costInCents.toFixed(2) + " R$</div>");
    ...
    $("#acceptDelivery").on("click", function() {
        if (deliveryHistory.length > 0 && !deliveryHistory[deliveryHistory.length - 1].accepted) {
            ...
            // Simulando o redirecionamento para a tela de pagamento
             window.location.href = "metodoPagar.html";  
            alert("Redirecionando para a tela de pagamento...");
            window.location.href = "metodoPagar.html";            // Adiciona a entrega aceita ao histórico de entregas pendentes
